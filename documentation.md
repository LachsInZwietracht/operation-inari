...
- **Version history:** When a plan transitions into `approved`, `useMealPlan` persists the live plan and appends a `meal_plan_versions` snapshot. Approved plans are read-only for slot editing/template application until reopened as `draft`; Planakte lists recent versions and can restore a version into an editable draft.
