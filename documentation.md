...
- **Nährstoff-Lückenfüller:** Tools-grid tile (`components/plan-nutrient-gap-tool.tsx` → dialog in `components/plan-nutrient-gap-dialog.tsx`). Pick one nutrient plus the amount still missing today — typed manually or prefilled via gap chips computed from the merged `dietLineCompliance`/`micronutrientCompliance` "low" items — and get catalog foods ranked by nutrient density with the exact gram portion that closes the gap ("120 g decken die Lücke"). Candidates come from `GET /api/foods/browser` in nutrient mode (`filter_foods_by_nutrient` RPC; the route now accepts `includePortions=1` and a whitelist-validated `nutrientIds` list for extra hydration); portion math, the realistic portion cap (max curated `food_portions` amount clamped to 20–400 g, else an energy-density heuristic), secondary constraints ("Nährstoff ≤/≥ Menge", hard = exclude, soft = score penalty, both evaluated at the computed portion, not per 100 g), and severe-allergen exclusion live as pure functions in `lib/nutrient-gap.ts` (fetch/caching in `hooks/use-nutrient-gap-search.ts` — amount edits recompute over cached candidates without refetching). Leaving the amount empty switches to research mode ("Was ist reich an Zink?", per-100 g display). "Übernehmen" inserts the computed portion into a selectable meal slot (default via `chooseOptimizationSlot`) through the same guarded add workflow. Known v1 limits: custom foods are absent (browser RPC runs with `requesting_user_id: null`) and multi-gap balancing is out of scope. Targeted E2E: `tests/nutrient-gap-tool.spec.ts`.
