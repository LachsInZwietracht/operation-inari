...
- **Two planner views sharing one Bibliothek:** The Tag and Woche tabs share a persistent left-column `MealPlanLibrary` (`components/meal-plan-library.tsx`) with Rezepte/Lebensmittel/Vorlagen tabs, search over the global food search index, and per-item kcal. Items are HTML5-draggable into either view (drag constants `MEAL_PLAN_DRAG_*` live in the library module) and offer a click-to-add "+" menu that targets a chosen meal slot of the active day; the Vorlagen tab applies a day template via the same `handleApplyTemplate` path as the Vorlagen dropdown. Building a plan therefore uses the same source and workflow in both views.
- **Tag view as Daten-Workspace:** `components/plan-day-workspace.tsx` renders weekday chips (Mo–So of the active week, with per-day kcal, click switches the date), a "Tag duplizieren" action (copies to the following day), and a dense nutrient table: one section per meal slot with totals row (kcal/Eiweiß/KH/Fett/Ballaststoffe), one row per entry with inline amount editing, allergen-warning tooltip, exchange and remove actions, plus a per-slot "Lebensmittel oder Rezept hinzufügen …" row that opens the add command. Slot sections accept drops from the Bibliothek and from other slot rows (`useMealPlan.moveEntry` cross-slot move; approved plans stay locked).
- **Sticky day-tab action header:** The day-tab toolbar (day navigation, Kostform picker, Vorlagen + Export dropdowns) is sticky at `top-14` (just below the app shell header) with a backdrop-blur background. The sticky bar also surfaces a compact allergen-conflict pill when conflicts exist, color-coded by the highest severity.
