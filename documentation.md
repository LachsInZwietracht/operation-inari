...
### 4.8 Patient List & Pipeline (`/patienten`) and Mail Merge (`/api-export`)
- **Pipeline status (`lib/patient-status.ts`):** every patient and every not-yet-applied invitation resolves to one state — `antwort_da` → `bereit` → `faellig` → `eingeladen` → `plan_aktiv` — derived from intake links, submissions, plan summaries, and the last counseling session. There is no stored status column; the derivation is the single source of truth and cannot drift from the records it describes. `buildPatientPipeline()` returns the unified row list, `countByStatus()` feeds the filter chips.
- **One list, no tabs:** `/patienten` no longer has `Patienten` / `Onboarding` / `Workflows` tabs. Invitations render as rows (`patient: null`) until their submission is applied, so onboarding is visible in the list instead of hidden behind a tab. Status chips act as filters and replace the old non-interactive worklist tiles. Rows are dense (`components/patient-pipeline-row.tsx`) and expose exactly one primary action derived from the status.
- **Handoff to the plan:** applying a submission opens a confirmation step offering `Plan für <Name> erstellen`, linking to `/ernaehrungsplan?patientId=<id>`. Because apply upserts `patient_allergens` and `patient_food_preferences` first, the plan editor arrives with the patient's allergens already loaded. The same handoff appears in `components/patient-intake-panel.tsx`.
...
- **Component:** `app/(app)/patienten/page.tsx`
  ...
  - **Patient rows:** the list derives `Letzte Beratung` from real `useCounseling()` session data instead of the legacy `COUNSELING_SESSIONS` mock constant, and shows it alongside the pipeline status.
...
- **Persistence:** `daily_meal_plans` and `meal_entries`. Clinical metadata added in migration `20260527000045_meal_plan_metadata.sql`; per-plan Diät-Line preset (`diet_line_id`) added in `20260528000046_meal_plan_diet_line.sql`; the `meal_plan_versions` table (`20260530000048_meal_plan_versions.sql`) remains in the schema but is no longer written or read after the approval/versioning feature was retired; patient-scoped uniqueness is enforced in `20260531000049_patient_scoped_meal_plan_uniqueness.sql`.
...
- **Patient context:** When opened with `patientId`, new plans inherit the patient context. The page header (`PageHeader` children in `ernaehrungsplan-client.tsx`) holds a slim patient selector that switches to another patient's plan for the same date or returns to the unassigned planner via `openPatientContext`, plus a **Zum Patienten** button that jumps to `/patienten/{id}`. There is no assignment-confirmation dialog and no manual title field — the plan title is derived automatically from the patient name. DGE/reference bars resolve against the patient’s date of birth, gender, and reference assignment; allergen warnings use the same patient context and multi-valued indications (joined with " · " in compact contexts).
- **Two planner views sharing one Bibliothek:** The Tag and Woche tabs share a persistent left-column `MealPlanLibrary` (`components/meal-plan-library.tsx`) with Rezepte/Lebensmittel/Vorlagen tabs, search over the global food search index, and per-item kcal. Items are HTML5-draggable into either view (drag constants `MEAL_PLAN_DRAG_*` live in the library module) and offer a click-to-add "+" menu that targets a chosen meal slot of the active day; the Vorlagen tab applies a day template via the same `handleApplyTemplate` path as the toolbar's **Aus Vorlage** button. Building a plan therefore uses the same source and workflow in both views.
...
- **Sticky day-tab action header:** The day-tab toolbar (day navigation, Kostform picker, Aus-Vorlage button, Export dropdown) is sticky at `top-14` (just below the app shell header) with a backdrop-blur background. The sticky bar also surfaces a compact allergen-conflict pill when conflicts exist, color-coded by the highest severity.
...
- **Autosave (no explicit save/approval):** Every edit — entries, amounts, moves, diet line, patient/date changes — persists immediately to Supabase through `useMealPlan`'s per-plan-serialized `syncPlanToSupabase`, so there is no manual `Ernährungsplan speichern` button. The former Planakte card (editable title, `Freigeben`/approval snapshot, `Als Vorlage speichern`, `Akte öffnen`, version history, manual checkpoints) was removed when the feature was simplified; plans stay editable drafts and are no longer approval-locked. The version-snapshot plumbing (`meal_plan_versions` client, `useMealPlanVersions`, `useMealPlan.approvePlan`/`reopenPlan`/`restorePlanVersion`/`createPlanCheckpoint`) was deleted with it.
- **Tagesbilanz & Vorschläge zum Auffüllen:** The day-view sidebar shows the Tagesbilanz (Ist/Soll/Δ with status-colored bars per Zielprofil target) and "Vorschläge zum Auffüllen" — deterministic food/recipe suggestions derived from low target values in the active Zielprofil, ranked by gap coverage, skipping severe patient-allergen conflicts, inserted through the same guarded add workflow.
...
- **Direct exports:** The day/week toolbar's **Export** button opens `components/plan-export-dialog.tsx` — a configurable PDF export instead of the former fixed three-option menu. The dialog offers document type (Patientenhandout · Klinischer Bericht · Lehrküchenplan), per-day selection via weekday chips (empty days disabled; defaults follow the active view: day view → active day, week view → all filled days), and handout-only content toggles: **Rezepte im Anhang** (deduplicated recipe cards with per-portion ingredients, instructions, kcal/portion, and "Geplant für …" day references) and **Kalorienziel & Ernährungsweise** (patient profile block from `Patient.dailyCalorieGoal`/`macroPreset`/`nutritionPreferences`/`patientGoals` plus allergen warnings; deliberately opt-in so handouts can omit kcal figures, e.g. for eating-disorder contexts) plus a free-text patient note. Handout and clinical variants render one PDF page per selected day (clinical adds per-day nutrient/vitamin/mineral tables) via `buildMealPlanDaysExportRequest()` in `lib/exports/report-builder.ts` and the multi-day layout in `lib/exports/pdf.tsx` (`dayPages`/`patientProfile`/`recipeAppendix` fields on `ReportExportRequest`); the Lehrküchenplan keeps its single-page week poster (`buildTeachingKitchenExportRequest`). Exports reuse `/api/exports/report` (file generation only; patient-bound report persistence was removed).
- **Plan templates:** `meal_plan_templates` (migration `20260529000047_meal_plan_templates.sql`) stores reusable day plans tagged by indication and diet line; system rows (`user_id IS NULL`) ship via the `etl:recipes` ETL from `lib/mock-data/meal-plan-templates.ts`. The day toolbar's **Aus Vorlage** button opens `PlanApplyTemplateDialog` (`useMealPlanTemplates`, filtered by any of the patient's indications / diet line); applying a template uses `useMealPlan.applyTemplateToDate`, replacing the day's slots and resetting status to `draft`. Authoring a personal template from the planner was removed — personal templates are now created from the Planvorlagen library's **Vorlage erstellen** (from a saved Ernährungsplan).
...
- **Food diary:** `/klient` shows one day (`?datum=YYYY-MM-DD`, defaults to today in `Europe/Berlin`), the five standard meal slots, and kcal/EW/F/KH totals. Entries live in `client_food_log_days` / `client_food_log_entries`, owned by the client. Food search uses the existing `search_foods` RPC; nutrients hydrate through `/api/foods/by-ids`. Data access: `lib/data/client-food-log-client.ts`, math in `lib/client-food-log.ts`.
  ...
  - **Registry exception.** This is the one place a client module reads another: `tagebuch → plan`, one way, every read behind `isClientModuleEnabled("plan")`. Switching the plan module off removes the planned rows and leaves the diary working.
  - **The day has a reference** (`lib/client-targets.ts`, `components/client/client-day-totals.tsx`). Four numbers with nothing to compare them to are decoration. Three sources in descending specificity: the **plan's own total for that date** (macros straight from the plan, not from a percentage split), then `patients.daily_calorie_goal` split by `patients.macro_preset`, then maintenance as basal rate × PAL. A fourth outcome — no reference at all — is respectable and deliberate: a client without a counselor is not handed a target the app invented. Those inputs reach the client through the same fixed projection as the measurement history (migration `20260811000079` adds an `energy` key to `client_patient_history()`); the basal rate is computed **in SQL** by Mifflin-St Jeor so date of birth and sex stay behind it. The bars have **no "over budget" state** — a diary that turns red when someone eats is a diary that gets deleted, and judging an overshoot is the counselor's call.
  - **Logging speed**, all derived from the log with no schema: household measures from `food_portions` as one-tap chips (the curated ETL has filled that table since the beginning and only the counselor side read it), with a typical portion as the default instead of a flat 100 g; a "Zuletzt oft" row of this slot's most frequent entries shown *before* anything is typed; "Wie gestern" on an empty slot whose predecessor was not empty; and amounts editable in place rather than delete-and-retype.
...
- **Module registry:** `lib/client-modules.ts` lists the client surface as modules (`tagebuch`, `plan`, `training`, `betreuung`) plus capabilities (`barcode`), which extend a module instead of owning a route. The bottom nav renders from it and each route guards itself with `isClientModuleEnabled`, so flipping `enabled` hides a module including its URL. Every module owns exactly five paths — route, components, `lib/data/client-<id>-client.ts`, migration, spec — and modules never import from each other, which makes removal mechanical.
- **Plan module (`/klient/plan`):** the client sees the plan their counselor shared for a date and answers each meal with eaten or skipped; tapping the active answer again clears it. Answers live in `client_meal_completions`, keyed uniquely per (client, meal entry). Only `active`/`approved` plans are exposed — drafts stay the counselor's workbench. Adherence per day appears in the counselor's **Klienten-App** tab.
- **Training module (`/klient/training`):** sessions with free-text exercises and sets (reps and/or weight). Progression is derived, never stored — `lib/client-training.ts` computes it from the sets themselves. Deliberately without an exercise catalog; the name is plain text and grouping is case-insensitive, so a catalog stays an additive migration later. Counselor access runs through `consent_training`, separate from the nutrition flag.
...
- **Not built yet:** importing a client log range into `nutrition_protocols`.
