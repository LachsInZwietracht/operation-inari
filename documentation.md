...
- **Module registry:** `lib/client-modules.ts` lists the client surface as modules (`tagebuch`, `plan`, `betreuung`). The bottom nav renders from it and each route guards itself with `isClientModuleEnabled`, so flipping `enabled` hides a module including its URL. Every module owns exactly five paths — route, components, `lib/data/client-<id>-client.ts`, migration, spec — and modules never import from each other, which makes removal mechanical.
...
- **Not built yet:** barcode scanning and importing a client log range into `nutrition_protocols`. See the milestones in `docs/client-mode-plan.md`.
