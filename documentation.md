...
- **Training module (`/klient/training`):** sessions with free-text exercises and sets (reps and/or weight); the set dialog shows the last performance of the same exercise while typing, which is the whole point of a training log. Progression is derived, never stored — `lib/client-training.ts` computes the best set per exercise per calendar week from the sets themselves. Deliberately without an exercise catalog; the name is plain text and grouping is case-insensitive, so a catalog stays an additive migration later. Counselor access runs through `consent_training`, separate from the nutrition flag.
