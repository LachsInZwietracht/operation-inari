...
- **Component:** `app/(app)/patienten/page.tsx`
  ...
  - **Dense worklist:** a compact patient worklist summarizes missing indications, overdue/missing counseling sessions, and upcoming birthdays above the patient cards.
  - **Page tabs:** `/patienten` separates the primary patient list from secondary utilities with top-level `Patienten` and `Workflows` tabs. The `Patienten` tab keeps search/filter, dense worklist, and patient cards first; the `Workflows` tab contains serial mailings and the birthday list.
