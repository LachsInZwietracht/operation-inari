...
- **Component:** `app/(app)/patienten/page.tsx`
  ...
  - **Page tabs:** `/patienten` separates the primary patient list from secondary utilities with top-level `Patienten` and `Workflows` tabs. The `Patienten` tab keeps search/filter, dense worklist, and patient cards first; the `Workflows` tab contains eGK demo intake, serial mailings, and the birthday list.
  - **eGK demo:** the patient overview exposes the simulated card-reader flow inside the `Workflows` tab's secondary `Patientenaufnahme` panel; the patient creation form still exposes clearly labeled simulated eGK flows for tests/product demos. Current Web Serial and companion paths still return demo card payloads rather than production connector data.
