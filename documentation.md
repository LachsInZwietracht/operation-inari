...
- **Patient context:** When opened with `patientId`, new plans inherit the patient context. The Planakte includes a patient selector for switching to another patient's plan for the same date, returning to the unassigned planner, or assigning an existing unassigned plan after confirmation. DGE/reference bars resolve against the patient’s date of birth, gender, and reference assignment; allergen warnings use the same patient context. Patient identity, indications (multi-valued — patients can have several at once, joined with " · " in compact contexts), reference standard, and the allergen-/intolerance count are folded into the Planakte card header (no longer a separate "Patientenkontext" card).
- **Sticky day-tab action header:** The day-tab toolbar (status pill, patient chip, day navigation, Kostform picker, Vorlagen + Export dropdowns, Rezepte trigger) is sticky at `top-14` (just below the app shell header) with a backdrop-blur background, so context and actions stay reachable while the user scrolls through the meal slots. The sticky bar also surfaces a compact allergen-conflict pill when conflicts exist, color-coded by the highest severity.
- **Recipe palette as Sheet drawer:** The "Rezepte" trigger in the sticky header opens a right-side `Sheet` (`components/ui/sheet.tsx`) instead of the previous cramped right-column card. The drawer has a search field, slot selector, category and sort selects (Name / kcal asc/desc / Zubereitungszeit), and patient-aware quick toggles ("Indikation passt" → matches `recipe.tags`/description against any of `patient.indications`; "Allergen-sicher" → hides recipes with `checkAllergenConflicts` matches). Each result card shows kcal/portion, prep+cook minutes, and an inline allergen-conflict badge. Adding routes through `guardedAddEntry` so severity-blocking rules apply.
- **Drag-and-drop between slots and reorder within slot:** Each entry exposes a `GripVertical` handle and is HTML5-draggable, carrying its source slot type and entry id (`MEAL_PLAN_DRAG_SOURCE_SLOT`/`MEAL_PLAN_DRAG_SOURCE_ENTRY`). `MealSlotCard` renders inter-entry drop zones that highlight while a move-source is hovering and call `onMoveEntry(sourceSlot, sourceEntryId, targetSlot, targetIndex)`; dropping on a card body without a specific gap moves the entry to the end of the target slot. `useMealPlan.moveEntry` performs the cross-slot move or within-slot reorder while preserving the entry id and amount, and respects the same lock guards as the other entry mutators (no-op when the plan is approved).
...
- **Planakte:** The day view exposes plan title, status (`draft`/`active`/`approved`/`archived`), notes, patient assignment state, and approval timestamp. Its primary actions are explicit workflow CTAs: `Ernährungsplan speichern` persists the current draft, `Freigeben` runs the clinical approval gate and creates the immutable approval snapshot, and `Abbrechen` leaves the planner without clearing the plan.
- **Clinical review gate:** Planakte derives a Freigabeprüfung from plan entries, patient assignment, selected target profile, allergen conflicts, and core meal coverage. Critical findings block `approved`; warnings remain visible but do not prevent approval.
- **Optimierungsassistent:** The day summary derives deterministic food/recipe suggestions from low target values in the active Zielprofil, ranks loaded candidates by gap coverage, skips severe patient-allergen conflicts, and inserts selected suggestions through the same guarded add workflow.
- **Version history:** When a plan transitions into `approved`, `useMealPlan` persists the live plan and appends a `meal_plan_versions` snapshot. Planakte can also write manual checkpoints for editable plans and records a `reopened` snapshot when a freigegebener Plan is reopened as `draft`. Approved plans are read-only for slot editing/template application until reopened; recent versions can be restored into an editable draft.
