-- Replace single `indication TEXT` column on patients with `indications TEXT[]`.
--
-- Patients commonly have more than one nutritional indication (e.g. Diabetes
-- and Adipositas). Modelling this as an array keeps the data structured for
-- filters, template matching and statistics instead of relying on
-- comma-separated free text.

ALTER TABLE patients
  ADD COLUMN indications TEXT[] NOT NULL DEFAULT '{}'::TEXT[];

UPDATE patients
SET indications = CASE
  WHEN indication IS NULL OR btrim(indication) = '' THEN '{}'::TEXT[]
  ELSE (
    SELECT COALESCE(
      ARRAY_AGG(btrim(part) ORDER BY ord) FILTER (WHERE btrim(part) <> ''),
      '{}'::TEXT[]
    )
    FROM unnest(string_to_array(indication, ',')) WITH ORDINALITY AS s(part, ord)
  )
END;

ALTER TABLE patients DROP COLUMN indication;

CREATE INDEX patients_indications_gin_idx ON patients USING GIN (indications);

-- Refresh the practice statistics summary so the top indications report
-- aggregates over the array instead of the dropped scalar column.
...
AS $$
  ...
  top_indications AS (
    ...
    FROM (
      SELECT btrim(indication_value) AS indication, COUNT(*)::integer AS count_value
      FROM patient_base, unnest(indications) AS indication_value
      WHERE btrim(indication_value) <> ''
      GROUP BY btrim(indication_value)
      ORDER BY count_value DESC
      LIMIT 5
    ) buckets
  )
