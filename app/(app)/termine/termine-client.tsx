...
export function TerminePageClient({
  ...
  const {
    ...
    updateAppointment,
    deleteAppointment,
  } = usePracticeAppointments({ initialAppointments })
  const { patients } = usePatients({ initialPatients })

  const [view, setView] = useState<ViewMode>("day")
  const [activeDate, setActiveDate] = useState<Date>(new Date())
  const [patientFilter, setPatientFilter] = useState<string>(initialPatientFilter ?? "alle")
