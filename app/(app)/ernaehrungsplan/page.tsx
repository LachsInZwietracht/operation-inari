...
function aggregatePlanNutrients(plan: DailyMealPlan): NutrientValue[] {
  const entryArrays: NutrientValue[][] = []
  for (const slot of plan.slots) {
    for (const entry of slot.entries) {
      entryArrays.push(calculateEntryNutrients(entry))
    }
  }
  return sumNutrients(entryArrays)
}

function getEntryLabel(entry: MealEntry): string {
  if (entry.type === "food") {
    const food = FOODS.find((f) => f.id === entry.referenceId)
...
export default function ErnaehrungsplanPage() {
  ...
  const filteredExchangeFoods = useMemo(() => {
    ...
    }).sort((a, b) => getNutrientValue(b.nutrients, exchangeNutrient) - getNutrientValue(a.nutrients, exchangeNutrient))
  }, [exchangeCategory, exchangeNutrient, exchangeSearch])
  ...
  const teachingKitchenRows = useMemo(() => {
    return weekPlans.map((plan) => {
      ...
      return {
        dateLabel,
        lunch: lunch && lunch.entries[0] ? getEntryLabel(lunch.entries[0]) : "noch offen",
        dinner: dinner && dinner.entries[0] ? getEntryLabel(dinner.entries[0]) : "Snack-/Buffetplanung",
      ...
      const highlights = Array.from(
        new Set(
          days
            ...
            .filter(Boolean)
            .map((entry) => getEntryLabel(entry as MealEntry)),
        ),
      ).slice(0, 3)
                        ...
                        <div key={slot.type} className="flex items-start justify-between gap-2">
                          ...
                          <span className="text-right">
                            {slot.entries.length > 0
                              ? slot.entries
                                  .map((entry) => getEntryLabel(entry).split("(")[0]?.trim())
                                  .slice(0, 2)
                                  .join(", ")
                              : "–"}
                          </span>
              ...
              <Table>
                ...
                <TableBody>
                  {filteredExchangeFoods.slice(0, 20).map((food) => {
                    const nutrientVal = getNutrientValue(food.nutrients, exchangeNutrient)
