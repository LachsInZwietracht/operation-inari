...
function getEntryEnergy(
  ...
  const nutrients = getEntryNutrientValues(entry, foodMap, recipeMap)
...
function getEntryCo2(
  ...
  if (entry.type === "food") {
    ...
    const energyKcal = getEntryEnergy(entry, foodMap, recipeMap)
...
export default function BerichtePage() {
  ...
  const planNutrients = useMemo(
    () => (selectedPlan ? calculatePlanNutrients(selectedPlan, foodMap, recipeMap) : []),
    [selectedPlan, foodMap, recipeMap],
  )
  ...
  const mealEnergyData = useMemo(() => {
    ...
    return selectedPlan.slots.map((slot) => {
      const energy = slot.entries.reduce(
        (sum, entry) => sum + getEntryEnergy(entry, foodMap, recipeMap),
        0,
      )
      return {
  ...
  const foodContributionData = useMemo(() => {
    ...
    for (const slot of selectedPlan.slots) {
      for (const entry of slot.entries) {
        const energy = getEntryEnergy(entry, foodMap, recipeMap)
        if (energy <= 0) continue
        const key = getEntryLabel(entry, foodMap, recipeMap)
    ...
    return [...contributions.entries()]
      ...
      .slice(0, 6)
  }, [selectedPlan, totalMealEnergy, foodMap, recipeMap])
  ...
  const planPreviewRows = useMemo(() => {
    ...
    return selectedPlan.slots.map((slot) => {
      const items = slot.entries
        .map((entry) => getEntryLabel(entry, foodMap, recipeMap))
        .join(", ")
      return {
  ...
  const co2BySlot = useMemo(() => {
    ...
    return selectedPlan.slots.map((slot) => {
      const value = slot.entries.reduce(
        (sum, entry) => sum + getEntryCo2(entry, foodMap, recipeMap),
        0,
      )
      return {
        name: MEAL_SLOT_LABELS[slot.type],
