...
interface ApiExportPageClientProps {
  initialExportJobs?: ExportJobRecord[]
  initialApiKeys?: ApiKeyRecord[]
}

export function ApiExportPageClient({ initialExportJobs, initialApiKeys }: ApiExportPageClientProps) {
  ...
  const [exportJobs, setExportJobs] = useState<ExportJobRecord[]>(initialExportJobs ?? [])
  ...
  useEffect(() => {
    if (!initialExportJobs) void loadExportJobs()
    if (!initialApiKeys) void loadApiKeys()
  }, [initialExportJobs, initialApiKeys])
