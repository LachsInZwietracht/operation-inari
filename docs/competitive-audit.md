...
- **Clinic IT readiness:** FHIR sync, SSO via SAML/OIDC, LDAP/Active Directory mapping, API keys, webhooks, integration persistence, and production-grade device connectors.
...
2. **Clinic IT readiness:** SSO, audit logs, FHIR, API keys, webhooks, and production connector foundations.
