...
| `client_daily_checkins` | One self-reported day: wellbeing (1–10), the optional sub-scores, the night before it, and alcohol. **Owner-only RLS — the counselor has no SELECT policy at all**, because sharing is decided per metric and a row policy cannot filter columns; the counselor path is `client_wellbeing_series()`. Every metric column is nullable and NULL means unanswered, never zero | `client_user_id`, `checkin_date` (unique per client), `wellbeing`, `energy`, `mood`, `digestion`, `sleep_minutes` (of the night ONTO the date), `sleep_quality`, `alcohol_units` (Standardgläser, no energy) |
| `client_metric_preferences` | Per-metric `tracked` / `shown` / `shared` switches. A row exists only where the client departed from the registry default in `lib/client-metrics.ts`, so an empty table is the normal case. `shared` narrows `consent_wellbeing`, never widens it | `client_user_id`, `metric_key` (no FK — the registry is TypeScript), `tracked`, `shown`, `shared` |
