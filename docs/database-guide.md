...
- Plan visibility runs the opposite direction to the food log: a plan is written *for* the client, so `client_can_read_patient_plans(patient_id)` gates it on an active link alone, not on a consent flag, and exposes only `active`/`approved` plans. It adds client-side SELECT policies to `daily_meal_plans`, `meal_entries`, and `recipes` (the last one scoped to recipes actually referenced by a readable plan, since personal recipes are otherwise owner-only). Covered by `tests/client-plan-rls.spec.ts`.
