...
- `co2PerPortion`, `sustainabilityScore` — sustainability (nullable)
...
| `20260512000029_database_lifecycle.sql` | Database lifecycle events, audited food-reference replacement logs, and the `replace_food_references()` RPC for user-workspace recipes, meal plans, and protocols (objects later dropped in `20260610000057`) |
...
| `20260820000097_meal_plan_release_revisions.sql` | Adds immutable patient/date release chains: one current release plus one change draft, atomic release/replacement/archive RPCs, append-only release snapshots, and draft-only direct-write RLS for plans and entries |
| `20260825000100_meal_plan_week_release.sql` | Adds `release_meal_plan_week_revision()`: validates and atomically releases exactly seven persisted, non-empty Monday-Sunday drafts while snapshotting each day |
| `20260826000001_meal_plan_template_day_blocks.sql` | Extends reusable meal-plan templates with optional patient-independent multi-day blocks stored as relative day offsets plus slot snapshots; existing single-day templates remain unchanged |
| `20260827000000_begin_meal_plan_week_revision.sql` | Adds `begin_meal_plan_week_revision()`: owner-checked, lock-protected and idempotent creation of the seven linked draft successors of a released week |
| `20260827000001_meal_plan_template_patient_scope.sql` | Adds optional `patient_id` to personal meal-plan templates. The FK cascades with its patient; a CHECK plus read/insert/update RLS require the authenticated counselor to own any bound patient. Existing system rows are retained but not part of the counselor template query. |
...
| `organization_data_source_settings` | Org-level on/off state for connected food databases (set in `/datenbank`; absence = active; owner/admin write, members read; gates food search) | `organization_id`, `source_id`, `is_active`, `updated_by` |
...
| `daily_meal_plans` | Daily plans scoped by patient context, with lifecycle metadata, selected target preset and immutable release chain | `user_id`, `date`, `patient_id`, `title`, `status`, `notes`, `target_profile_id`, `diet_line_id`, `approved_at`, `approved_by`, `revision_number`, `supersedes_plan_id`, `replaced_at` |
...
| `meal_plan_templates` | Counselor-owned reusable day or relative multi-day blueprints | `user_id`, optional `patient_id` (NULL = reusable for the counselor's patients), `source_type`, `slots`, `day_blocks` |
...
| `client_workout_sessions` | One training session, client-owned | `client_user_id`, `session_date`, `title`, `notes` |
| `client_workout_sets` | Sets within a session. No exercise catalog: `exercise_name` is free text, grouped case-insensitively for progression | `session_id`, `client_user_id`, `exercise_name`, `set_index`, `reps`, `weight_kg` (at least one of the two required) |
| `client_meal_completions` | Client answers to a planned meal: eaten or skipped. A missing row means no reaction, which the adherence view must not read as a skip | `client_user_id`, `meal_plan_id`, `meal_entry_id` (unique per client+entry), `skipped`, `completed_at` |
| `client_daily_checkins` | One self-reported day: Energie, Stimmung and Verdauung (1–5), the night before it, and alcohol. An overall wellbeing score (1–10) was dropped in migration `…096`. **Owner-only RLS — the counselor has no SELECT policy at all**, because sharing is decided per metric and a row policy cannot filter columns; the counselor path is `client_wellbeing_series()`. Every metric column is nullable and NULL means unanswered, never zero | `client_user_id`, `checkin_date` (unique per client), `energy`, `mood`, `digestion`, `sleep_minutes` (of the night ONTO the date), `sleep_quality`, `alcohol_units` (Standardgläser, no energy) |
| `client_metric_preferences` | Per-metric switches. `tracked` and `shown` are two columns behind one switch in the UI — written together, read as visible only when both are true, kept apart so rows from when they were separate keep their meaning. A row exists only where the client departed from the registry default in `lib/client-metrics.ts`, so an empty table is the normal case. `shared` narrows `consent_wellbeing`, never widens it | `client_user_id`, `metric_key` (no FK — the registry is TypeScript), `tracked`, `shown`, `shared` |
...
### Client Check-in Read Paths

- `client_daily_checkins` and `client_metric_preferences` are owner-only. There is no counselor policy on either, by design.
- `client_wellbeing_series(target_patient, from_date, to_date)` is the counselor's only read: `SECURITY DEFINER`, checks the active link plus `consent_wellbeing`, applies the per-metric `shared` switch, and returns long format `(checkin_date, metric_key, value)`. A metric that is not shared produces no rows, and a column added to the table stays invisible until it is added to the function's explicit `VALUES` list.
- The function's fallback when no preference row exists is `TRUE`, mirroring the `shared: true` defaults in `lib/client-metrics.ts`. Those two are one decision written twice; changing the registry default means changing the `COALESCE`.
...
- Plan visibility runs the opposite direction to the food log: a plan is written *for* the client, so `client_can_read_patient_plans(patient_id)` gates it on an active link alone, not on a consent flag, and exposes only the one current `active`/`approved` release per patient/date. The patient workflow prepares revisions with `begin_meal_plan_week_revision()`: it owner-checks, advisory-locks and row-locks one Monday–Sunday range, then atomically creates seven linked drafts (or idempotently returns the complete existing successor week). Those drafts stay private while the seven former releases remain visible; `release_meal_plan_week_revision()` replaces them atomically. The older single-day lifecycle RPCs remain authenticated owner-checked compatibility boundaries. Direct counselor writes to released plan rows and entries are denied by RLS. Covered by `tests/client-plan-rls.spec.ts`.
...
- `patient_intake_submissions` (`20260803000073`, review extension `20260812000087`) — the raw validated payload as JSONB, plus `status` (`new` / `reviewed` / `applied` / `discarded`), `applied_patient_id`, `reviewer_notes`, `reviewed_at`, and `reviewed_by`. Corrected review data replaces the JSON payload only during the authenticated apply action. Discarding keeps the response for traceability and removes it from the active intake queue. A unique index on `link_id` enforces one submission per invitation, mirroring `20260608000055` for digital protocols. Practitioner RLS resolves ownership through `patient_intake_links`. No anon INSERT policy — inserts go through `/api/onboarding/submit` with the service role.
...
- Created `fetchAllFoodsForList()` — fetches selected display nutrients without portions (~2-3 MB)
...
`supabase/migrations/20260604000054_filter_foods_by_nutrient.sql` adds `filter_foods_by_nutrient()`, which powers the foods browser's "Nach Nährstoff sortieren & filtern" panel (PRODI-feedback #4) and the meal planner's Nährstoff-Lückenfüller tool. It joins `food_nutrients` for a single `nutrient_id`, normalizes the amount to a per-100 g basis (`amount * 100 / NULLIF(per_amount, 0)` — `per_amount` is 100 for BLS/SFK rows but can differ for branded foods), applies the same source/category/group/custom-visibility filters as `search_foods_with_total`, optional `min_per_100g`/`max_per_100g` thresholds, and orders by the normalized amount (`asc`/`desc`). A composite `idx_food_nutrients_nutrient_amount (nutrient_id, amount)` index keeps the per-nutrient scan efficient. `group_filter` is a `TEXT[]` so the browser can pass `getFoodGroupDescendants(groupId)`. `fetchFoodsBrowserPageByNutrient` in `lib/data/foods.ts` calls it and falls back to the direct query path if the function is absent. In nutrient mode `FoodBrowserQuery` additionally supports `includePortions` (hydrate `food_portions` on the results) and `extraNutrientIds` (hydrate up to 12 nutrient columns beyond the default browser subset); `/api/foods/browser` exposes them as `includePortions=1` and a comma-separated `nutrientIds` list validated against `NUTRIENT_DEFINITIONS`.
...
**Checking migration status:** Query `SELECT proname FROM pg_proc WHERE proname = 'search_foods_with_total'` to confirm the paginated search RPC is present.
...
**Env loading & target selection.** `playwright.config.ts` manually parses `.env.test` (Playwright doesn't use Next.js env loading) and requires it to point at local Supabase (`http://127.0.0.1:54321` or `localhost`). The suite stops before discovery when the file is missing or the URL is external. A deliberately approved throwaway cloud project can be supplied through process env or `.env.local` only with `ALLOW_E2E_EXTERNAL_SUPABASE=true`; never set that flag for production or routine local work. Existing `process.env` values win over values from a loaded file.

The config logs which file and Supabase host it loaded at startup, e.g. `[playwright] Loaded env from .env.test → Supabase http://127…`, so you can confirm the target before a run. It also fixes the worker count at one because the fixtures share this project. `.env.test` is gitignored (`.env*`).
